#Seasonal plotting for Trout Bog 2020 (only 3 days available) 
...
path.to.files <- "/Users/cnolmsted/Desktop/BogData2020/"
FilesToLoad <- list.files(path = path.to.files)
TB_all_2020 <- read.csv(file=paste0(path.to.files,FilesToLoad[1]))

for (file in 2:length(FilesToLoad)){
  my_file <- read.csv(file=paste0(path.to.files,FilesToLoad[file]))
  TB_all_2020 <- rbind(TB_all_2020, my_file)
  print(file)
}

TB_all_2020$date_lubridate <- parse_date_time(TB_all_2020$DATE, "mdy")

colnames(TB_all_2020)
TB_All <-TB_all_2020[ c(5:25) ]
TB_All <- TB_All[which(TB_All$Depth < 6.8), ]
...
estimate_Turbidity_by_date(ymd("2020-06-17"), c(0, 1, 1.5, 2))
...
Turbidity_interp_depth <- crossing(
  ...
  tibble(depth = seq(0, 6.75, length.out = 250))
) %>%
  group_by(date) %>%
...
estimate_Turbidity_by_depth(
  target_depth = 0, 
  target_date = seq(ymd("2020-06-17"), ymd("2020-07-28"), by = 1))


Turbidity_raster <- crossing(
  # dates can now be any value
  tibble(date = seq(ymd("2020-06-17"), ymd("2020-08-19"), by = 1)),
...
############
#ORP time (But there were pH problems so we cant trust ORP)
############
...
estimate_ORP_by_date(ymd("2020-06-17"), c(0, 1, 1.5, 2))
...
estimate_ODO.1_by_date(ymd("2020-06-17"), c(0, 1, 1.5, 2))
...
estimate_Temp_by_date(ymd("2020-06-17"), c(0, 1, 1.5, 2))
...
estimate_pH_by_date(ymd("2020-06-17"), c(0, 1, 1.5, 2))
...
estimate_Cond_by_date(ymd("2020-06-17"), c(0, 1, 1.5, 2))
