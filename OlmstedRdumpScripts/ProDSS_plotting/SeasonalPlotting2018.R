#Seasonal plotting for Trout Bog 2018
...
path.to.files <- "/Users/cnolmsted/Desktop/BogData2018/"
...
for (file in 2:length(FilesToLoad)){
  ...
  TB_all_2018 <- rbind(TB_all_2018, my_file)
  print(file)
}

TB_all_2018$date_lubridate <- parse_date_time(TB_all_2018$DATE, "mdy")

colnames(TB_all_2018)
TB_All <-TB_all_2018[ c(5:25) ]
TB_All <- TB_All[which(TB_All$Depth < 6.5), ]
TB_All <- TB_All[which(TB_All$Depth > 0.28), ]
...
estimate_Turbidity_by_date(ymd("2018-06-21"), c(0, 1, 1.5, 2))
...
Turbidity_interp_depth <- crossing(
  ...
  tibble(depth = seq(.3, 6, length.out = 300))
) %>%
  group_by(date) %>%
...
estimate_Turbidity_by_depth(
  target_depth = 0.3, 
  target_date = seq(ymd("2018-06-21"), ymd("2018-06-25"), by = 1))


Turbidity_raster <- crossing(
  # dates can now be any value
  tibble(date = seq(ymd("2018-06-21"), ymd("2018-08-28"), by = 1)),
...
estimate_ORP_by_date(ymd("2018-06-21"), c(0, 1, 1.5, 2))
...
estimate_ODO.1_by_date(ymd("2018-06-21"), c(0, 1, 1.5, 2))
...
estimate_Temp_by_date(ymd("2018-06-21"), c(0, 1, 1.5, 2))
...
estimate_pH_by_date(ymd("2018-06-21"), c(0, 1, 1.5, 2))
...
estimate_Cond_by_date(ymd("2018-06-21"), c(0, 1, 1.5, 2))
